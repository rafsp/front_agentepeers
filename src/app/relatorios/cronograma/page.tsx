// src/app/relatorios/cronograma/page.tsx
'use client'
...
import { CronogramaReport } from '@/components/report'
import { reportsService } from '@/lib/api/reports-service'
import type { CronogramaReportData } from '@/types/reports'

function CronogramaContent(): React.ReactElement {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { loading: authLoading, isAuthenticated } = useAuth()
  
  const [data, setData] = useState<CronogramaReportData | null>(null)
  ...
  const loadData = async (): Promise<void> => {
    ...
    try {
      const response = await reportsService.getCronograma(projectId)
      setData(response)
    } catch (err) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-500 mb-4">Selecione um projeto na pagina de relatorios para visualizar o cronograma.</p>
  ...
  return (
    <div className="relative">
      ...
      <CronogramaReport data={data} />
    </div>
  )
}

export default function CronogramaPage(): React.ReactElement {
  return (
    <Suspense fallback={
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
