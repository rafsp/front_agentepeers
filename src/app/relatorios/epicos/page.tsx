// src/app/relatorios/epicos/page.tsx
'use client'
...
import { EpicosReport } from '@/components/report'
import { reportsService } from '@/lib/api/reports-service'
import type { EpicosReportData } from '@/types/reports'

function EpicosContent(): React.ReactElement {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { loading: authLoading, isAuthenticated } = useAuth()
  
  const [data, setData] = useState<EpicosReportData | null>(null)
  ...
  const loadData = async (): Promise<void> => {
    ...
    try {
      const response = await reportsService.getEpicos(projectId)
      setData(response)
    } catch (err) {
      console.error('Erro ao carregar epicos:', err)
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-500">Carregando relatorio de epicos...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-500 mb-4">Selecione um projeto na pagina de relatorios para visualizar os epicos.</p>
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-500 mb-4">{error || 'Nao foi possivel carregar os dados.'}</p>
  ...
  return (
    <div className="relative">
      ...
      <EpicosReport data={data} />
    </div>
  )
}

export default function EpicosPage(): React.ReactElement {
  return (
    <Suspense fallback={
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
